...
export class ValidationResult implements IValidationResult {
  validateResult(event: APIGatewayProxyEvent): {
    ...
    let validator = ValidationFactory.create();
    let data = this._mapFileds(multipart.parse(event, false));
    let validationResult = validator.validate(data);
  ...
  _mapFileds(parsed: any): IFormData {
    return {
      ...
      from: parsed.from,
      to: !!parsed.to && parsed.to.trim().length > 0 ? parsed.to.split(",") : [],
      cc: !!parsed.cc && parsed.cc.trim().length > 0 ? parsed.cc.split(",") : [],
      bcc: !!parsed.bcc && parsed.bcc.trim().length > 0 ? parsed.bcc.split(",") : [],
